...
describes what users get. Pick a format with `-o`, or let metmuseum choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
metmuseum <command> -o table   # aligned columns for reading
metmuseum <command> -o jsonl   # one JSON object per line, for piping
metmuseum <command> -o json    # a single JSON array
metmuseum <command> -o csv     # spreadsheet friendly
metmuseum <command> -o tsv     # tab-separated
metmuseum <command> -o url     # just the URL column
metmuseum <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
metmuseum <command> --fields id,title,url
```
...
```bash
metmuseum <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
metmuseum <command>            # a table, because this is a terminal
metmuseum <command> | wc -l    # JSONL, because this is a pipe
```
